...
func Register(r *server.Hertz) {
	...
	root := r.Group("/", rootMw()...)
	{
		...
		{
			_rag := _knowledge.Group("/rag", _ragMw()...)
			...
			// ========== 知识库基础管理接口 (FastGPT兼容) ==========
			{
				_core := _rag.Group("/core", _ragcoreMw()...)
				{
					...
					// ========== FastGPT标准集合管理接口 ==========
					{
						...
						_collection.POST("/", append(_createcollectionfastgptMw(), coze.CreateCollectionFastGPT)...)
						_collection.GET("/", append(_getcollectionsfastgptMw(), coze.GetCollectionsFastGPT)...)
						_collection.GET("/:id", append(_getcollectionbyidfastgptMw(), coze.GetCollectionByIdFastGPT)...)
						_collection.PUT("/:id", append(_updatecollectionfastgptMw(), coze.UpdateCollectionFastGPT)...)
						_collection.DELETE("/:id", append(_deletecollectionfastgptMw(), coze.DeleteCollectionFastGPT)...)
					...
					// ========== FastGPT标准数据管理接口 ==========
					{
						_data := _dataset.Group("/data", _ragcoredataMw()...)
						_data.POST("/", append(_insertdatafastgptMw(), coze.InsertDataFastGPT)...)
						_data.POST("/push", append(_pushdatafastgptMw(), coze.PushDataFastGPT)...)
						_data.GET("/", append(_getdatalistfastgptMw(), coze.GetDataListFastGPT)...)
						_data.GET("/:id", append(_getdatabyidfastgptMw(), coze.GetDataByIdFastGPT)...)
						_data.PUT("/:id", append(_updatedatafastgptMw(), coze.UpdateDataFastGPT)...)
						_data.DELETE("/:id", append(_deletedatafastgptMw(), coze.DeleteDataFastGPT)...)
					}
				}
			}
			
			// ========== 原有接口 ==========
			_rag.POST("/search", append(_ragsearchMw(), coze.RagSearch)...)
			_rag.POST("/search/deep", append(_deepsearchMw(), coze.DeepSearch)...)
			
			// ========== 数据集管理接口 ==========
			{
				_dataset := _rag.Group("/dataset", _ragdatasetMw()...)
				_dataset.GET("/list", append(_getragdatasetsMw(), coze.GetRagDatasets)...)
				_dataset.GET("/:id", append(_getragdatasetbyidMw(), coze.GetRagDatasetById)...)
				_dataset.PUT("/:id", append(_updateragdatasetMw(), coze.UpdateRagDataset)...)
				_dataset.DELETE("/:id", append(_deleteragdatasetMw(), coze.DeleteRagDataset)...)
				_dataset.POST("/create", append(_createragdatasetMw(), coze.CreateRagDataset)...)
				// 删除同步API - Coze直接通过知识库ID调用RAG，不需要数据同步
				_dataset.POST("/retrain", append(_retrainragdatasetMw(), coze.RetrainRagDataset)...)
			}
			
			// ========== 集合管理接口 ==========
			{
				_collection := _rag.Group("/collection", _ragcollectionMw()...)
				_collection.GET("/list", append(_getragcollectionsMw(), coze.GetRagCollections)...)
				_collection.GET("/:id", append(_getragcollectionbyidMw(), coze.GetRagCollectionById)...)
				_collection.PUT("/:id", append(_updateragcollectionMw(), coze.UpdateRagCollection)...)
				_collection.DELETE("/:id", append(_deleteragcollectionMw(), coze.DeleteRagCollection)...)
				_collection.POST("/create", append(_createragcollectionMw(), coze.CreateRagCollection)...)
				_collection.POST("/:id/sync", append(_syncragcollectionMw(), coze.SyncRagCollection)...)
				_collection.POST("/:id/retrain", append(_retrainragcollectionMw(), coze.RetrainRagCollection)...)
				_collection.GET("/:id/training", append(_getragcollectiontrainingdetailMw(), coze.GetRagCollectionTrainingDetail)...)
				_collection.GET("/:id/export", append(_exportragcollectionMw(), coze.ExportRagCollection)...)
				
				// 文件处理接口
				{
					_create := _collection.Group("/create", _ragcollectioncreateMw()...)
					_create.POST("/file", append(_createragcollectionfromfileMw(), coze.CreateRagCollectionFromFile)...)
					_create.POST("/link", append(_createragcollectionfromlinkMw(), coze.CreateRagCollectionFromLink)...)
					_create.POST("/text", append(_createragcollectionfromtextMw(), coze.CreateRagCollectionFromText)...)
				}
				
				// 批量操作接口
				{
					_batch := _collection.Group("/batch", _ragcollectionbatchMw()...)
					_batch.POST("/create", append(_batchcreateragcollectionsMw(), coze.BatchCreateRagCollections)...)
					_batch.POST("/delete", append(_batchdeleteragcollectionsMw(), coze.BatchDeleteRagCollections)...)
					_batch.POST("/retrain", append(_batchretraincollectionsMw(), coze.BatchRetrainCollections)...)
				}
			}
			
			// ========== 数据管理接口 ==========
			{
				_data := _rag.Group("/data", _ragdataMw()...)
				_data.POST("/", append(_insertragdataMw(), coze.InsertRagData)...)
				_data.POST("/push", append(_pushragdatabatchMw(), coze.PushRagDataBatch)...)
				_data.GET("/list", append(_getragdatalistMw(), coze.GetRagDataList)...)
				_data.GET("/:id", append(_getragdatabyidMw(), coze.GetRagDataById)...)
				_data.PUT("/:id", append(_updateragdataMw(), coze.UpdateRagData)...)
				_data.DELETE("/:id", append(_deleteragdataMw(), coze.DeleteRagData)...)
				
				// 批量数据操作
				{
					_batch := _data.Group("/batch", _ragdatabatchMw()...)
					_batch.POST("/update", append(_batchupdateragdataMw(), coze.BatchUpdateRagData)...)
				}
			}
			
			// ========== 文件处理接口 ==========
			{
				_file := _rag.Group("/file", _ragfileMw()...)
				_file.POST("/upload", append(_uploadragfileMw(), coze.UploadRagFile)...)
				_file.POST("/process", append(_processragfileMw(), coze.ProcessRagFile)...)
				_file.GET("/types", append(_getsupportedfiletypesMw(), coze.GetSupportedFileTypes)...)
				_file.POST("/validate", append(_validateragfileMw(), coze.ValidateRagFile)...)
			...
			// ========== 高级搜索接口 ==========
			{
				_search := _rag.Group("/search", _ragsearchgroupMw()...)
				_search.POST("/test", append(_ragsearchtestMw(), coze.RagSearchTest)...)
				_search.POST("/advanced", append(_ragadvancedsearchMw(), coze.RagAdvancedSearch)...)
				_search.POST("/hybrid", append(_raghybridsearchMw(), coze.RagHybridSearch)...)
				_search.POST("/rerank", append(_ragsearchwithrerankMw(), coze.RagSearchWithRerank)...)
			}
			
			// ========== 训练管理接口 ==========
			{
				_training := _rag.Group("/training", _ragtrainingMw()...)
				_training.POST("/start", append(_startragtrainingMw(), coze.StartRagTraining)...)
				_training.GET("/status", append(_getragtrainingstatusMw(), coze.GetRagTrainingStatus)...)
			}
			
			// ========== 监控管理接口 ==========
			_rag.GET("/health", append(_getraghealthMw(), coze.GetRagHealth)...)
			_rag.GET("/usage", append(_getragusagestatsMw(), coze.GetRagUsageStats)...)
			_rag.GET("/metrics", append(_getragmetricsMw(), coze.GetRagMetrics)...)
			_rag.GET("/audit", append(_getragauditlogsMw(), coze.GetRagAuditLogs)...)
			_rag.GET("/export", append(_exportragdataMw(), coze.ExportRagData)...)
			_rag.GET("/performance", append(_getragperformancestatsMw(), coze.GetRagPerformanceStats)...)
		}
		}
		{
