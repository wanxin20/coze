...
const modifierKey: any = {
  ctrl: (event: KeyboardEvent) => !isAppleDevice && event.ctrlKey,
  control: (event: KeyboardEvent) => isAppleDevice && event.ctrlKey,
 ...
 * @param exactMatch
 */
export function isKeyStringMatch(
  event: KeyboardEvent,
  keyString: string,
  exactMatch = true,
): boolean {
