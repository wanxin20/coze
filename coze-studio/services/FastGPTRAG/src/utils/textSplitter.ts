...
// 默认最大块大小
const defaultMaxChunkSize = 16000;

// 获取文本有效长度（简化版）
const getTextValidLength = (text: string): number => {
  return text.length;
};

// 简化文本处理
const simpleText = (text: string): string => {
  return text
    .replace(/\s+/g, ' ')
    .replace(/\n\s*\n\s*\n/g, '\n\n')
    .trim();
...
const commonSplit = (props: SplitProps): SplitResponse => {
  ...
  try {
    ...
    logger.error('Text splitting error:', err);
    throw new Error(`Text splitting failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
  }
};
